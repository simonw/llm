...
def test_logs_localtime_flag():
    """Regression test for #1161: -L/--localtime flag converts UTC to local time."""
    from click.testing import CliRunner
    from llm.cli import cli
    import re

    runner = CliRunner()
    runner.invoke(cli, ["-m", "echo", "test localtime flag"])

    # Test with -L flag (should contain a timezone offset like +05:30 or -04:00)
    result_local = runner.invoke(cli, ["logs", "-n", "1", "-L"])
    assert result_local.exit_code == 0

    # Verify that the local output contains a timezone indicator
    assert re.search(r"[+-]\d{2}:\d{2}", result_local.output), "Local time should include timezone offset"
