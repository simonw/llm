...
def keys_get(name):
    ...
    Outputs the actual key value to stdout, which is useful for exporting
    to environment variables or using in scripts.
...
def templates_list():
    ...
    Shows all templates you've created, including a preview of their system prompt
    and regular prompt content. Templates are stored as YAML files that you can
    edit directly or modify with the 'llm templates edit' command.
    ...
    \b
        • Left side shows the template name (use with -t option)
        • System prompts define the model's behavior and personality  
        • Regular prompts are the main instruction with optional variables
        • Variables like $input are replaced when you use the template

    🎯 Using Templates:
    
    \b
        llm -t template-name 'your input here'     # Use template
        llm -t template-name -p var1 value        # Pass parameters  
        llm chat -t template-name                  # Start chat with template

    📚 Related Commands:
    
    \b
        • llm templates edit <name>       # Modify existing template
        • llm templates path              # Show template directory
        • llm --save <name>               # Create template from prompt

    📚 Documentation:
    
    \b
        • Template Usage: https://llm.datasette.io/en/stable/templates.html#using-a-template
        • Creating Templates: https://llm.datasette.io/en/stable/templates.html#getting-started-with-save
    """
    path = template_dir()
    pairs = []
