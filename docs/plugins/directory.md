...
- **[llm-multi](https://github.com/yaniv-aknin/llm-multi)** adds `llm archive` command for bundling multiple files (into JSONL, XML etc) and `llm map` to concurrently process multiple inputs using an LLM prompt.
