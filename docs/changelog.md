...
- `llm logs --json` now shows which configured toolbox instance served each tool result - the toolbox name, plugin and constructor arguments, so a `SQLite_query` result records that it ran against `SQLite("mydb.db")`. Recorded in the new `tool_instantiations` table for new logs, and read from the legacy `tool_instances` data for history logged by older versions of LLM, which recorded it without ever displaying it. [#1562](https://github.com/simonw/llm/pull/1562)
