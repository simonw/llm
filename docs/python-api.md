...
To log a response, use `log_response()`. This applies any pending schema migrations and then performs the same write as the CLI, so everything you log shows up in `llm logs`:
